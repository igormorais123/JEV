"""Terceiro juiz cego nos desacordos do corpus de replicação (E12).

Mesmo procedimento do E8b e do E11, sob a regra escrita no pré-registro do E12 antes de existir
qualquer dado: o juiz recebe só a mensagem e as duas leituras, **em ordem sorteada**, sem saber
qual veio de quem e sem ver nenhuma resposta de modelo.
...
Uso:
    python -m executor.adjudicar_e12              # gera casos-cegos.json e prompt-adjudicacao.txt
    python -m executor.adjudicar_e12 --consolidar bruta.jsonl
"""
import argparse
import json
import random
...
def consolidar(caminho_bruto, juiz):
    """Consolida as decisões do juiz cego contra o mapa que ficou fora do prompt."""
    ...
    saida = {'at': datetime.now(timezone.utc).isoformat(), 'terceiro_juiz': juiz,
             ...
             'corpus': 'data/corpus/triagem-replicacao.jsonl',
...
def main():
    ...
    parser.add_argument('--juiz', default='terceiro juiz cego, outro fornecedor')
    ...
    if args.consolidar:
        ...
        for c in saida['casos']:
            print('  %-14s autor=%-11s local=%-11s -> %s (%s)'
    ...
    print('Prompt em runs/e12-replicacao/prompt-adjudicacao.txt')
