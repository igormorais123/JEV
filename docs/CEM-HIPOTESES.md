...
**Mediu:** 19804 — 19804 de 27396 decisões em que a confiança difere da probabilidade da classe escolhida; maior diferença 0.50 (escolheu `reversivel` com probabilidade 0.5 e confiança 0)
...
**Mediu:** 0,2186 — 5990 de 27396 decisões com 1,0
...
**Mediu:** 5680,7 — p99 sobre 22476 chamadas respondidas; máximo 32256 ms. Incluindo os timeouts o p99 vira 7498 ms, que é a constante de timeout do cliente e não uma latência do modelo
...
|---|---|---|---|---|
| `H001` | Mandar duas perguntas no mesmo payload não custa mais latência que mandar uma. | razão entre as medianas < 1,3 | **sustentada** | 1,034 |
| `H002` | O custo de uma chamada é essencialmente linear no tamanho do payload. | r de Pearson > 0,90 | **sustentada** | 0,9854 |
| `H003` | O tamanho do estado domina o custo: o resto do payload é ruído contábil. | r de Pearson > 0,85 | **sustentada** | 0,9384 |
| `H004` | A latência é dominada por custo fixo, não pelo tamanho do que se manda. | r de Pearson < 0,30 | **sustentada** | -0,0121 |
...
| `H012` | A confiança relatada é a probabilidade da classe escolhida, não um número à parte. | nenhuma divergência acima de 0,01 | **FALSIFICADA** | 19804 |

- **H001** · fonte: livro-caixa, 3.180 recibos de decisão · mediana 614 ms com duas perguntas contra 594 ms com uma, em 4920 e 17556 chamadas
- **H002** · fonte: livro-caixa, recibos com custo e bytes · r de Pearson sobre 22476 recibos
- **H003** · fonte: livro-caixa, recibos · r de Pearson sobre 22476 recibos
- **H004** · fonte: livro-caixa, recibos · r de Pearson sobre 22611 recibos; latência mediana 601 ms
...
- **H011** · fonte: todos os artefatos com gabarito de classes · 27396 decisões conferidas contra os critérios declarados na própria chamada
- **H012** · fonte: R4-R7, campo `probabilidades` · 19804 de 27396 decisões em que a confiança difere da probabilidade da classe escolhida; maior diferença 0.50 (escolheu `reversivel` com probabilidade 0.5 e confiança 0)
...
| `H015` | A confiança é degenerada: 1,0 é de longe o valor mais comum. | fração com confiança == 1,0 acima de 0,50 | **FALSIFICADA** | 0,2186 |
...
- **H015** · fonte: livro-caixa, 3.180 decisões · 5990 de 27396 decisões com 1,0
...
| | hipótese | previa | veredito | mediu |
|---|---|---|---|---|
| `H027` | A distribuição de probabilidade quase sempre colapsa numa classe só. | fração degenerada > 0,70 | **FALSIFICADA** | 0,2473 |
...
| `H032` | A segunda opção quase nunca recebe massa. | fração com segunda == 0 acima de 0,70 | **FALSIFICADA** | 0,2473 |
...
- **H027** · fonte: livro-caixa, vetores de probabilidade das 3.180 decisões · 6774 de 27396 vetores
...
- **H031** · fonte: R4-R7 e livro-caixa · 27636 vetores conferidos
- **H032** · fonte: livro-caixa, vetores de probabilidade · 6774 de 27396
...
| | hipótese | previa | veredito | mediu |
|---|---|---|---|---|
| `H037` | A latência mediana cabe dentro de um passo interativo. | mediana < 700 | **sustentada** | 598,4 |
| `H038` | A cauda de latência não estoura o orçamento de um gancho de editor. | p99 < 3000 | **FALSIFICADA** | 5680,7 |
| `H039` | A latência tem cauda longa em relação à mediana. | p99/p50 > 3 | **sustentada** | 9,49 |
...
| `H042` | O gasto do estudo está concentrado em poucas rodadas. | fração das três maiores > 0,80 | **sustentada** | 0,9639 |
| `H043` | Ordenar contexto custa menos que responder com ele. | custo médio de ordenação menor | **FALSIFICADA** | -0 |
| `H044` | Dobrar o estado aproximadamente dobra o custo. | razão > 2 | **FALSIFICADA** | 1,42 |
| `H045` | Chamadas com duas perguntas não custam o dobro. | razão < 1,5 | **sustentada** | 1,197 |
| `H046` | O paralelismo de oito linhas não degradou a latência. | razão < 1,5 | **sustentada** | 1,095 |
| `H047` | A taxa de falha de transporte do estudo inteiro é baixa. | taxa de erro < 0,03 | **sustentada** | 0,011 |
| `H048` | O modelo devolve pouquíssimo token de saída: o preço zero de saída não é sorte. | mediana < 200 | **sustentada** | 63 |

- **H037** · fonte: livro-caixa, 3.180 recibos · 22476 chamadas respondidas
- **H038** · fonte: livro-caixa, recibos de chamadas respondidas (emenda de 2026-09-20) · p99 sobre 22476 chamadas respondidas; máximo 32256 ms. Incluindo os timeouts o p99 vira 7498 ms, que é a constante de timeout do cliente e não uma latência do modelo
- **H039** · fonte: livro-caixa, recibos de chamadas respondidas (emenda de 2026-09-20) · p99 5681 ms sobre mediana 598 ms, em chamadas respondidas
- **H040** · fonte: livro-caixa, recibos · mediana de 22476 chamadas
...
- **H042** · fonte: livro-caixa, agrupado por experimento · shared-lab 69.0%, shared-e15 24.6%, shared-router 2.8%
...
- **H044** · fonte: livro-caixa, recibos · quartil superior 33373 nUSD contra inferior 23564 nUSD
- **H045** · fonte: livro-caixa, recibos · 27847 nUSD contra 23269 nUSD
- **H046** · fonte: livro-caixa, recibos por consumidor · paralelo 602 ms em 22406 contra sequencial 549 ms em 184
- **H047** · fonte: livro-caixa, tabela attempts · 266 falhas em 24281 tentativas: success 23851, reserved 164, timeout 119, http_error 107, sent 32, invalid_response 8
- **H048** · fonte: livro-caixa, tabela attempts com uso registrado · mediana sobre 22476 recibos; máximo 134
