...
|---|---|---|---|---|
| `H001` | Mandar duas perguntas no mesmo payload não custa mais latência que mandar uma. | razão entre as medianas < 1,3 | **sustentada** | 1,031 |
| `H002` | O custo de uma chamada é essencialmente linear no tamanho do payload. | r de Pearson > 0,90 | **sustentada** | 0,9798 |
| `H003` | O tamanho do estado domina o custo: o resto do payload é ruído contábil. | r de Pearson > 0,85 | **sustentada** | 0,9367 |
| `H004` | A latência é dominada por custo fixo, não pelo tamanho do que se manda. | r de Pearson < 0,30 | **sustentada** | -0,0108 |
...
| `H015` | A confiança é degenerada: 1,0 é de longe o valor mais comum. | fração com confiança == 1,0 acima de 0,50 | **FALSIFICADA** | 0,2175 |
...
| | hipótese | previa | veredito | mediu |
|---|---|---|---|---|
| `H027` | A distribuição de probabilidade quase sempre colapsa numa classe só. | fração degenerada > 0,70 | **FALSIFICADA** | 0,2463 |
...
| `H032` | A segunda opção quase nunca recebe massa. | fração com segunda == 0 acima de 0,70 | **FALSIFICADA** | 0,2463 |
...
| `H042` | O gasto do estudo está concentrado em poucas rodadas. | fração das três maiores > 0,80 | **sustentada** | 0,9641 |
