...
**Mediu:** 19716 — 19716 de 27287 decisões em que a confiança difere da probabilidade da classe escolhida; maior diferença 0.50 (escolheu `reversivel` com probabilidade 0.5 e confiança 0)
...
**Mediu:** 0,2188 — 5971 de 27287 decisões com 1,0
...
**Mediu:** 5693,8 — p99 sobre 22389 chamadas respondidas; máximo 32256 ms. Incluindo os timeouts o p99 vira 7501 ms, que é a constante de timeout do cliente e não uma latência do modelo
...
|---|---|---|---|---|
| `H001` | Mandar duas perguntas no mesmo payload não custa mais latência que mandar uma. | razão entre as medianas < 1,3 | **sustentada** | 1,035 |
| `H002` | O custo de uma chamada é essencialmente linear no tamanho do payload. | r de Pearson > 0,90 | **sustentada** | 0,9853 |
| `H003` | O tamanho do estado domina o custo: o resto do payload é ruído contábil. | r de Pearson > 0,85 | **sustentada** | 0,9373 |
| `H004` | A latência é dominada por custo fixo, não pelo tamanho do que se manda. | r de Pearson < 0,30 | **sustentada** | -0,0125 |
...
| `H012` | A confiança relatada é a probabilidade da classe escolhida, não um número à parte. | nenhuma divergência acima de 0,01 | **FALSIFICADA** | 19716 |

- **H001** · fonte: livro-caixa, 3.180 recibos de decisão · mediana 614 ms com duas perguntas contra 594 ms com uma, em 4898 e 17491 chamadas
- **H002** · fonte: livro-caixa, recibos com custo e bytes · r de Pearson sobre 22389 recibos
- **H003** · fonte: livro-caixa, recibos · r de Pearson sobre 22389 recibos
- **H004** · fonte: livro-caixa, recibos · r de Pearson sobre 22524 recibos; latência mediana 600 ms
...
- **H011** · fonte: todos os artefatos com gabarito de classes · 27287 decisões conferidas contra os critérios declarados na própria chamada
- **H012** · fonte: R4-R7, campo `probabilidades` · 19716 de 27287 decisões em que a confiança difere da probabilidade da classe escolhida; maior diferença 0.50 (escolheu `reversivel` com probabilidade 0.5 e confiança 0)
...
| `H015` | A confiança é degenerada: 1,0 é de longe o valor mais comum. | fração com confiança == 1,0 acima de 0,50 | **FALSIFICADA** | 0,2188 |
...
- **H015** · fonte: livro-caixa, 3.180 decisões · 5971 de 27287 decisões com 1,0
...
| | hipótese | previa | veredito | mediu |
|---|---|---|---|---|
| `H027` | A distribuição de probabilidade quase sempre colapsa numa classe só. | fração degenerada > 0,70 | **FALSIFICADA** | 0,2474 |
...
| `H032` | A segunda opção quase nunca recebe massa. | fração com segunda == 0 acima de 0,70 | **FALSIFICADA** | 0,2474 |
...
- **H027** · fonte: livro-caixa, vetores de probabilidade das 3.180 decisões · 6751 de 27287 vetores
...
- **H031** · fonte: R4-R7 e livro-caixa · 27527 vetores conferidos
- **H032** · fonte: livro-caixa, vetores de probabilidade · 6751 de 27287
...
| | hipótese | previa | veredito | mediu |
|---|---|---|---|---|
| `H037` | A latência mediana cabe dentro de um passo interativo. | mediana < 700 | **sustentada** | 597,6 |
| `H038` | A cauda de latência não estoura o orçamento de um gancho de editor. | p99 < 3000 | **FALSIFICADA** | 5693,8 |
| `H039` | A latência tem cauda longa em relação à mediana. | p99/p50 > 3 | **sustentada** | 9,53 |
...
| `H042` | O gasto do estudo está concentrado em poucas rodadas. | fração das três maiores > 0,80 | **sustentada** | 0,9644 |
| `H043` | Ordenar contexto custa menos que responder com ele. | custo médio de ordenação menor | **FALSIFICADA** | -0 |
| `H044` | Dobrar o estado aproximadamente dobra o custo. | razão > 2 | **FALSIFICADA** | 1,41 |
| `H045` | Chamadas com duas perguntas não custam o dobro. | razão < 1,5 | **sustentada** | 1,197 |
| `H046` | O paralelismo de oito linhas não degradou a latência. | razão < 1,5 | **sustentada** | 1,262 |
...
- **H037** · fonte: livro-caixa, 3.180 recibos · 22389 chamadas respondidas
- **H038** · fonte: livro-caixa, recibos de chamadas respondidas (emenda de 2026-09-20) · p99 sobre 22389 chamadas respondidas; máximo 32256 ms. Incluindo os timeouts o p99 vira 7501 ms, que é a constante de timeout do cliente e não uma latência do modelo
- **H039** · fonte: livro-caixa, recibos de chamadas respondidas (emenda de 2026-09-20) · p99 5694 ms sobre mediana 598 ms, em chamadas respondidas
- **H040** · fonte: livro-caixa, recibos · mediana de 22389 chamadas
...
- **H042** · fonte: livro-caixa, agrupado por experimento · shared-lab 69.3%, shared-e15 24.7%, shared-router 2.5%
...
- **H044** · fonte: livro-caixa, recibos · quartil superior 33164 nUSD contra inferior 23558 nUSD
- **H045** · fonte: livro-caixa, recibos · 27847 nUSD contra 23269 nUSD
- **H046** · fonte: livro-caixa, recibos por consumidor · paralelo 602 ms em 22406 contra sequencial 477 ms em 117
- **H047** · fonte: livro-caixa, tabela attempts · 266 falhas em 24194 tentativas: success 23764, reserved 164, timeout 119, http_error 107, sent 32, invalid_response 8
- **H048** · fonte: livro-caixa, tabela attempts com uso registrado · mediana sobre 22389 recibos; máximo 134
