...
**Mediu:** 29204 — 29204 de 44177 decisões em que a confiança difere da probabilidade da classe escolhida; maior diferença 0.50 (escolheu `reversivel` com probabilidade 0.5 e confiança 0)
...
**Mediu:** 0,2778 — 12378 de 44561 decisões com 1,0
...
**Mediu:** 6261,3 — p99 sobre 31540 chamadas respondidas; máximo 32256 ms. Incluindo os timeouts o p99 vira 7369 ms, que é a constante de timeout do cliente e não uma latência do modelo
...
|---|---|---|---|---|
| `H001` | Mandar duas perguntas no mesmo payload não custa mais latência que mandar uma. | razão entre as medianas < 1,3 | **sustentada** | 0,927 |
...
| `H012` | A confiança relatada é a probabilidade da classe escolhida, não um número à parte. | nenhuma divergência acima de 0,01 | **FALSIFICADA** | 29204 |

- **H001** · fonte: livro-caixa, 3.180 recibos de decisão · mediana 642 ms com duas perguntas contra 692 ms com uma, em 5577 e 23822 chamadas
- **H002** · fonte: livro-caixa, recibos com custo e bytes · r de Pearson sobre 31540 recibos
- **H003** · fonte: livro-caixa, recibos · r de Pearson sobre 31540 recibos
- **H004** · fonte: livro-caixa, recibos · r de Pearson sobre 31675 recibos; latência mediana 716 ms
...
- **H011** · fonte: todos os artefatos com gabarito de classes · 44177 decisões conferidas contra os critérios declarados na própria chamada
- **H012** · fonte: R4-R7, campo `probabilidades` · 29204 de 44177 decisões em que a confiança difere da probabilidade da classe escolhida; maior diferença 0.50 (escolheu `reversivel` com probabilidade 0.5 e confiança 0)
...
| `H015` | A confiança é degenerada: 1,0 é de longe o valor mais comum. | fração com confiança == 1,0 acima de 0,50 | **FALSIFICADA** | 0,2778 |
...
- **H015** · fonte: livro-caixa, 3.180 decisões · 12378 de 44561 decisões com 1,0
...
| | hipótese | previa | veredito | mediu |
|---|---|---|---|---|
| `H027` | A distribuição de probabilidade quase sempre colapsa numa classe só. | fração degenerada > 0,70 | **FALSIFICADA** | 0,3086 |
...
| `H032` | A segunda opção quase nunca recebe massa. | fração com segunda == 0 acima de 0,70 | **FALSIFICADA** | 0,3086 |
...
- **H027** · fonte: livro-caixa, vetores de probabilidade das 3.180 decisões · 13750 de 44561 vetores
...
- **H031** · fonte: R4-R7 e livro-caixa · 44801 vetores conferidos
- **H032** · fonte: livro-caixa, vetores de probabilidade · 13750 de 44561
...
| | hipótese | previa | veredito | mediu |
|---|---|---|---|---|
| `H037` | A latência mediana cabe dentro de um passo interativo. | mediana < 700 | **FALSIFICADA** | 713,1 |
| `H038` | A cauda de latência não estoura o orçamento de um gancho de editor. | p99 < 3000 | **FALSIFICADA** | 6261,3 |
| `H039` | A latência tem cauda longa em relação à mediana. | p99/p50 > 3 | **sustentada** | 8,78 |
...
| `H042` | O gasto do estudo está concentrado em poucas rodadas. | fração das três maiores > 0,80 | **sustentada** | 0,9598 |
...
| `H045` | Chamadas com duas perguntas não custam o dobro. | razão < 1,5 | **sustentada** | 1,194 |
| `H046` | O paralelismo de oito linhas não degradou a latência. | razão < 1,5 | **sustentada** | 0,665 |
| `H047` | A taxa de falha de transporte do estudo inteiro é baixa. | taxa de erro < 0,03 | **sustentada** | 0,0082 |
| `H048` | O modelo devolve pouquíssimo token de saída: o preço zero de saída não é sorte. | mediana < 200 | **sustentada** | 63 |

- **H037** · fonte: livro-caixa, 3.180 recibos · 31540 chamadas respondidas
- **H038** · fonte: livro-caixa, recibos de chamadas respondidas (emenda de 2026-09-20) · p99 sobre 31540 chamadas respondidas; máximo 32256 ms. Incluindo os timeouts o p99 vira 7369 ms, que é a constante de timeout do cliente e não uma latência do modelo
- **H039** · fonte: livro-caixa, recibos de chamadas respondidas (emenda de 2026-09-20) · p99 6261 ms sobre mediana 713 ms, em chamadas respondidas
...
- **H042** · fonte: livro-caixa, agrupado por experimento · shared-lab 75.1%, shared-e15 16.3%, shared-router 4.6%
...
- **H044** · fonte: livro-caixa, recibos · quartil superior 61275 nUSD contra inferior 28045 nUSD
- **H045** · fonte: livro-caixa, recibos · 27889 nUSD contra 23352 nUSD
- **H046** · fonte: livro-caixa, recibos por consumidor · paralelo 689 ms em 29794 contra sequencial 1035 ms em 1005
- **H047** · fonte: livro-caixa, tabela attempts · 274 falhas em 33415 tentativas: success 32915, reserved 226, timeout 119, http_error 107, sent 40, invalid_response 8
- **H048** · fonte: livro-caixa, tabela attempts com uso registrado · mediana sobre 31540 recibos; máximo 2507
