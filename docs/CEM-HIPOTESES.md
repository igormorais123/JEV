...
**Mediu:** 29786 — 29786 de 44869 decisões em que a confiança difere da probabilidade da classe escolhida; maior diferença 0.50 (escolheu `reversivel` com probabilidade 0.5 e confiança 0)
...
**Mediu:** 0,2754 — 12464 de 45253 decisões com 1,0
...
**Mediu:** 6261,3 — p99 sobre 32158 chamadas respondidas; máximo 32256 ms. Incluindo os timeouts o p99 vira 7352 ms, que é a constante de timeout do cliente e não uma latência do modelo
...
|---|---|---|---|---|
| `H001` | Mandar duas perguntas no mesmo payload não custa mais latência que mandar uma. | razão entre as medianas < 1,3 | **sustentada** | 0,914 |
| `H002` | O custo de uma chamada é essencialmente linear no tamanho do payload. | r de Pearson > 0,90 | **sustentada** | 0,9889 |
| `H003` | O tamanho do estado domina o custo: o resto do payload é ruído contábil. | r de Pearson > 0,85 | **sustentada** | 0,9537 |
| `H004` | A latência é dominada por custo fixo, não pelo tamanho do que se manda. | r de Pearson < 0,30 | **sustentada** | 0,0209 |
...
| `H012` | A confiança relatada é a probabilidade da classe escolhida, não um número à parte. | nenhuma divergência acima de 0,01 | **FALSIFICADA** | 29786 |

- **H001** · fonte: livro-caixa, 3.180 recibos de decisão · mediana 644 ms com duas perguntas contra 705 ms com uma, em 5651 e 24366 chamadas
- **H002** · fonte: livro-caixa, recibos com custo e bytes · r de Pearson sobre 32158 recibos
- **H003** · fonte: livro-caixa, recibos · r de Pearson sobre 32158 recibos
- **H004** · fonte: livro-caixa, recibos · r de Pearson sobre 32300 recibos; latência mediana 730 ms
...
- **H011** · fonte: todos os artefatos com gabarito de classes · 44869 decisões conferidas contra os critérios declarados na própria chamada
- **H012** · fonte: R4-R7, campo `probabilidades` · 29786 de 44869 decisões em que a confiança difere da probabilidade da classe escolhida; maior diferença 0.50 (escolheu `reversivel` com probabilidade 0.5 e confiança 0)
...
| `H015` | A confiança é degenerada: 1,0 é de longe o valor mais comum. | fração com confiança == 1,0 acima de 0,50 | **FALSIFICADA** | 0,2754 |
...
- **H015** · fonte: livro-caixa, 3.180 decisões · 12464 de 45253 decisões com 1,0
...
*10 hipóteses: 4 falsificada, 6 sustentada.*

| | hipótese | previa | veredito | mediu |
|---|---|---|---|---|
| `H027` | A distribuição de probabilidade quase sempre colapsa numa classe só. | fração degenerada > 0,70 | **FALSIFICADA** | 0,3062 |
...
| `H032` | A segunda opção quase nunca recebe massa. | fração com segunda == 0 acima de 0,70 | **FALSIFICADA** | 0,3062 |
...
- **H027** · fonte: livro-caixa, vetores de probabilidade das 3.180 decisões · 13856 de 45253 vetores
...
- **H031** · fonte: R4-R7 e livro-caixa · 45493 vetores conferidos
- **H032** · fonte: livro-caixa, vetores de probabilidade · 13856 de 45253
...
*12 hipóteses: 3 falsificada, 1 inconclusiva, 8 sustentada.*

| | hipótese | previa | veredito | mediu |
|---|---|---|---|---|
| `H037` | A latência mediana cabe dentro de um passo interativo. | mediana < 700 | **FALSIFICADA** | 725,6 |
| `H038` | A cauda de latência não estoura o orçamento de um gancho de editor. | p99 < 3000 | **FALSIFICADA** | 6261,3 |
| `H039` | A latência tem cauda longa em relação à mediana. | p99/p50 > 3 | **sustentada** | 8,63 |
...
| `H042` | O gasto do estudo está concentrado em poucas rodadas. | fração das três maiores > 0,80 | **sustentada** | 0,9621 |
| `H043` | Ordenar contexto custa menos que responder com ele. | custo médio de ordenação menor | **FALSIFICADA** | -0 |
| `H044` | Dobrar o estado aproximadamente dobra o custo. | razão > 2 | **sustentada** | 2,19 |
| `H045` | Chamadas com duas perguntas não custam o dobro. | razão < 1,5 | **sustentada** | 1,192 |
| `H046` | O paralelismo de oito linhas não degradou a latência. | razão < 1,5 | **sustentada** | 0,636 |
| `H047` | A taxa de falha de transporte do estudo inteiro é baixa. | taxa de erro < 0,03 | **sustentada** | 0,0083 |
| `H048` | O modelo devolve pouquíssimo token de saída: o preço zero de saída não é sorte. | mediana < 200 | **sustentada** | 63 |

- **H037** · fonte: livro-caixa, 3.180 recibos · 32158 chamadas respondidas
- **H038** · fonte: livro-caixa, recibos de chamadas respondidas (emenda de 2026-09-20) · p99 sobre 32158 chamadas respondidas; máximo 32256 ms. Incluindo os timeouts o p99 vira 7352 ms, que é a constante de timeout do cliente e não uma latência do modelo
- **H039** · fonte: livro-caixa, recibos de chamadas respondidas (emenda de 2026-09-20) · p99 6261 ms sobre mediana 726 ms, em chamadas respondidas
...
- **H042** · fonte: livro-caixa, agrupado por experimento · shared-lab 70.9%, shared-e15 15.4%, shared-router 9.9%
...
- **H044** · fonte: livro-caixa, recibos · quartil superior 61200 nUSD contra inferior 27981 nUSD
- **H045** · fonte: livro-caixa, recibos · 27889 nUSD contra 23394 nUSD
- **H046** · fonte: livro-caixa, recibos por consumidor · paralelo 689 ms em 29794 contra sequencial 1083 ms em 1630
- **H047** · fonte: livro-caixa, tabela attempts · 282 falhas em 34047 tentativas: success 33533, reserved 232, timeout 126, http_error 107, sent 41, invalid_response 8
- **H048** · fonte: livro-caixa, tabela attempts com uso registrado · mediana sobre 32158 recibos; máximo 2507
