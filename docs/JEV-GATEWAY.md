...
Com o corte em 0,75, a mesma tarefa no Claude Code saiu assim: turno 1 `hint` Read (Jev 0,85, 2.340 ms, 20.297 tokens; o modelo leu e fez o Grep no mesmo turno), turno 2 `passthrough` porque o Jev ficou em 0,50 entre "responder" e "chamar ferramenta" (o turno em que só falta escrever a resposta é o mais ambíguo para ele, e o corte fez o que devia). Resposta correta, 2 requisições, 227.076 tokens de entrada (194.614 em cache) e 648 de saída.

**Codex: não medido.** A mesma tarefa de dois passos travou três vezes em 300 s (com e sem roteamento) sem que nenhuma requisição chegasse ao gateway, enquanto "responda oi" pelo mesmo caminho responde em ~45 s (a partida dos MCPs do `config.toml` domina esse tempo) e a rodada de uma linha das 03:27 (seção 3) funcionou. É comportamento do `codex exec` nesta máquina, não do gateway; fica registrado para quem repetir a medição usar o `jev-gateway-bench` (`--agent codex`), que roda com configuração limpa.
...
- **Codex: não passe `-c` extra depois de `jev-codex`.** Conferido em 2026-09-22: `jev-codex exec -c 'mcp_servers={}' "..."` respondeu normalmente, mas nenhuma requisição passou pelo gateway (o `-c` do usuário anula os `-c model_provider=…` que o lançador injeta). Sem a flag, a mesma chamada aparece no log. Quem precisa desligar MCPs deve fazê-lo no `config.toml`.
...
- **Painel:** `jev-codex --dashboard` (ou `localhost:8793/dashboard`) mostra os dois gateways; a página do Claude (`localhost:8789/dashboard`) só vê o Codex se receber `?peers=8793`, porque ele está fora da porta padrão. Conferido no navegador em 2026-09-22: cartões com "min confidence 0.9", os dois provedores e as requisições registradas.
